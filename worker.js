...
async function getQueuesFromCityUrl(cityUrl, env) {
  ...
  // 1️⃣ Основний пошук
  const direct = [...html.matchAll(/href="([^"]*\/cherha-[^"]+)"/g)];
  ...
  // 2️⃣ Fallback (Київ / великі міста)
  const base = new URL(cityUrl).pathname.replace(/\/$/, "");

  const streetLinks = [...html.matchAll(/href="([^"]+)"/g)]
    .map(m => m[1])
    .filter(h =>
      h.startsWith(base + "/") &&
      !h.includes("cherha-") &&
  ...
  for (const h of streetLinks) {
    const streetUrl = h.startsWith("http")
      ? h
      : "https://bezsvitla.com.ua" + h;
    ...
    const matches = [...page.matchAll(/href="([^"]*\/cherha-[^"]+)"/g)];
