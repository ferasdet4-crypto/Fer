...
// === FIXED: search deduplication added ===
// Changes:
// 1) Per-user st.lastSearch prevents duplicate identical searches
// 2) searchCities() results deduplicated by url
...
async function searchCities(query, env) {
  ...
  data
    ...
    .forEach(x => { if (!uniq.has(x.url)) uniq.set(x.url, { name: x.name, url: x.url }); });
...
async function getQueuesFromCityUrl(cityUrl, env) {
  ...
  try {
    ...
    // ===== 1️⃣ СПРОБА: шукати cherha-* прямо на сторінці міста
    const directMatches = [
    ...
    // ===== 2️⃣ ФОЛБЕК ДЛЯ КИЄВА: шукати вулиці / райони
    const base = new URL(cityUrl).pathname.replace(/\/$/, "");
  ...
  .filter(h => {
    ...
    if (h.includes("cherha-")) return false;
    if (h.includes("#")) return false;
    if (h.includes("oblast")) return false;
    if (h.split("/").length > 4) return false; // глибокі SEO-сторінки
    return true;
  });
    ...
    const streetUrls = [
      ...new Set(
        streetMatches
          .map(h => h.startsWith("http") ? h : "https://bezsvitla.com.ua" + h)
          .slice(0, 25) // ⚠️ ЛІМІТ
      )
    ];
    ...
    for (const streetUrl of streetUrls) {
      try {
        const r = await fetch(streetUrl, {
          headers: {
            "User-Agent": "Mozilla/5.0",
            "Accept-Language": "uk-UA,uk;q=0.9"
          }
        ...
        // ігноруємо помилки окремих вулиць
      }
    }
    ...
    console.log("getQueuesFromCityUrl error:", e);
    return [];
  } finally {
    clearTimeout(t);
  }
...
function extractLiItems(html) {
  ...
  const re = /<li[^>]*class="([^"]*)"[^>]*>\s*(\d{2}:\d{2})\s*[–-]\s*(\d{2}:\d{2})\s*<\/li>/gi;
  ...
  while ((m = re.exec(html)) !== null) {
    ...
    let on = null;
    if (cls.includes("on")) on = true;
    if (cls.includes("off")) on = false;
...
async function handleMessage(message, token, env) {
  ...
  // prevent duplicate identical search
  if (st.lastSearch === text) {
    await saveUserState(env, key, st);
    return;
  }
  st.lastSearch = text;
