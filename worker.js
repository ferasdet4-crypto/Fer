...
const VERSION = "kv-cron-final-1.1.0";
...
async function searchCities(query, env) {
  ...
  return data
    ...
    .map(x => ({ name: x.name, url: normalizeBezUrl(x.url) }))
    .slice(0, 8);
}

async function getQueuesFromCityUrl(cityUrl, env) {
  const absCityUrl = normalizeBezUrl(cityUrl);
  ...
  // 1) Best: find queue URLs anywhere in HTML (Kyiv часто має /cherha-* в script/data, не тільки в <a href>)
  const urlSet = new Set();

  // Full URLs
  for (const m of html.matchAll(/https?:\/\/bezsvitla\.com\.ua\/[^"'<> \n\r\t]*cherha-\d+(?:-\d+)+[^"'<> \n\r\t]*/gi)) {
    urlSet.add(m[0].split(/[#"']/)[0]);
  }
  // Relative paths
  for (const m of html.matchAll(/\/[^"'<> \n\r\t]*cherha-\d+(?:-\d+)+[^"'<> \n\r\t]*/gi)) {
    urlSet.add(normalizeBezUrl(m[0].split(/[#"']/)[0]));
  }

  let urls = [...urlSet].filter(u => String(u).includes("/cherha-"));
  if (urls.length) {
    urls = [...new Set(urls.map(u => normalizeBezUrl(u)))];

    const parseNums = (u) => {
      const tail = String(u).split("cherha-")[1] || "";
      const raw = tail.split(/[/?#]/)[0];
      const parts = raw.split("-").map(x => parseInt(x, 10)).filter(n => Number.isFinite(n));
      return parts.length ? parts : [999, 999, 999];
    };

    urls.sort((a, b) => {
      const A = parseNums(a), B = parseNums(b);
    ...
    return urls.map(u => {
      const raw = String(u).split("cherha-")[1].split(/[/?#]/)[0];
      const code = raw.replace(/-/g, ".");
      return { name: `Черга ${code}`, url: u };
    });
  }

  // 2) Fallback: some localities require going into districts/streets pages
  const basePath = new URL(absCityUrl).pathname.replace(/\/$/, "");

  const pageUrls = [...new Set(
    [...html.matchAll(/href="([^"]+)"/g)]
      .map(m => m[1])
      .filter(h => h && !h.includes("#") && !h.toLowerCase().startsWith("javascript:"))
      .map(h => normalizeBezUrl(h))
      .filter(u => {
        try {
          const p = new URL(u).pathname.replace(/\/$/, "");
          return p.startsWith(basePath + "/") && !p.includes("cherha-");
        } catch { return false; }
      })
      .slice(0, 40)
  )];
  ...
  for (const pageUrl of pageUrls) {
    try {
      ...
      const found = new Set();
      for (const m of h.matchAll(/https?:\/\/bezsvitla\.com\.ua\/[^"'<> \n\r\t]*cherha-\d+(?:-\d+)+[^"'<> \n\r\t]*/gi)) {
        found.add(m[0].split(/[#"']/)[0]);
      }
      for (const m of h.matchAll(/\/[^"'<> \n\r\t]*cherha-\d+(?:-\d+)+[^"'<> \n\r\t]*/gi)) {
        found.add(normalizeBezUrl(m[0].split(/[#"']/)[0]));
      }

      for (const u0 of found) {
        const u = normalizeBezUrl(u0);
        if (seen.has(u)) continue;
        seen.add(u);
        const raw = String(u).split("cherha-")[1].split(/[/?#]/)[0];
        const code = raw.replace(/-/g, ".");
        queues.push({ name: `Черга ${code}`, url: u });
      }

      if (queues.length >= 60) break;
    } catch {
  ...
  const parseCode = (name) => {
    const s = String(name).replace(/[^\d.]/g, "");
    const parts = s.split(".").map(x => parseInt(x, 10)).filter(n => Number.isFinite(n));
    return parts.length ? parts : [999, 999];
  };

  queues.sort((a, b) => {
    const A = parseCode(a.name), B = parseCode(b.name);
    const L = Math.max(A.length, B.length);
    for (let i = 0; i < L; i++) {
      const da = A[i] ?? 0, db = B[i] ?? 0;
      if (da !== db) return da - db;
