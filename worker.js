...
const VERSION = "kv-cron-final-1.2.1-fullfix";
...
async function loadUserState(env, key) {
  ...
  if (!("lastSearch" in obj)) obj.lastSearch = null;
...
async function getQueuesFromCityUrl(cityUrl, env) {
  const isKyiv = /kyiv|kiev|київ/i.test(cityUrl);
  ...
  try {
    ...
    if (directMatches.length && !isKyiv) {
      const urls = [
        ...new Set(
          directMatches.map(m =>
            m[1].startsWith("http")
...
function extractLiItems(html) {
  ...
  while ((m = re.exec(html)) !== null) {
    ...
    const end = tm[2];
    let on = null;
    if (/💡|lamp|light[-_\s]?on|bulb/i.test(li)) on = true;
    if (/❌|✖|xmark|light[-_\s]?off|no[-_\s]?light/i.test(li)) on = false;
