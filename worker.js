...
async function getQueuesFromCityUrl(cityUrl, env) {
   ...
   * ПІДТРИМУЄ:
   * - /kyiv/cherha-1-1
   * - https://bezsvitla.com.ua/kyiv/cherha-1-1
   * - картки з id="queue-1-1"
   */

  const set = new Map();

  // 1️⃣ Лінки
  const linkRegex = /href="(https:\/\/bezsvitla\.com\.ua\/[^"]*\/cherha-[^"]+|\/[^"]*\/cherha-[^"]+)"/g;
  for (const m of html.matchAll(linkRegex)) {
    let url = m[1];
    if (url.startsWith("/")) url = "https://bezsvitla.com.ua" + url;

    const code = url.split("cherha-")[1].replace(/-/g, ".");
    set.set(url, { name: `Черга ${code}`, url });
  }

  // 2️⃣ fallback: id="queue-X-Y"
  const idRegex = /id="queue-(\d+)-(\d+)"/g;
  for (const m of html.matchAll(idRegex)) {
    const code = `${m[1]}.${m[2]}`;
    const url = `${cityUrl.replace(/\/$/, "")}/cherha-${m[1]}-${m[2]}`;
    set.set(url, { name: `Черга ${code}`, url });
  }
